...
- **Islands UI**. Title bar / activity bar / status bar sit flat on a per-theme vertical
  ...
  the layout JetBrains shipped in IntelliJ 2025. A single **Edge gap** slider in Settings
  drives every chrome padding so the whole layout tightens or loosens in lockstep.
...
- **Live preview** via Jewel's Markdown renderer with GitHub-flavored extensions: tables,
  alerts, ~~strikethrough~~, autolinks. Ctrl+Click opens links in the system browser.
...
- **Native file dialogs**: the Open File, Save File and Open Folder pickers all use the
  native OS dialog on macOS (NSOpenPanel / NSSavePanel) and on Windows/Linux. No Swing
  `JFileChooser` UI is ever shown on macOS.
...
- **Settings dialog** (Appearance / Editor / Behavior / Shortcuts / About): theme,
  background gradient presets, panel corners, panel spacing, edge gap, editor font and size,
  word wrap, status bar, heap-usage indicator, session restore, exit on last-tab-close,
  OS-decorated fallback. Shortcuts are fully rebindable; defaults follow the platform —
  `⌘…` on macOS, `Ctrl+…` everywhere else.
- **Status bar** with caret position, selection length, dirty state, document metrics, and
  JVM heap usage (clickable to GC).
